echo "sample_id	#CVDJ	#VDJ	#total_read	#max_read	#unrecomb	#bad_reads" > final_summary.rpt
...
summarize() {
    ...
    if [ -s recombine_results/${1}.1.bad.rpt ]; then
        ...
        a=$(python3 summarize_good_report.py -lr  recombine_results/${1}.1.good.rpt \
                                             ...
                                             -bad recombine_results/${1}.2.bad.rpt \
                                             -out recombine_results/${1}.good.rpt)
        b=$(wc -l recombine_results/${1}.2.bad.rpt | awk '{print $1}')
        echo ${1}"	""${a}""	"${b} >> final_summary.rpt
    else
        # The file is empty.
        echo "[AIRRsembly] All alignments are good!"
        a=$(python3 summarize_good_report.py -lr  recombine_results/${1}.1.good.rpt \
                                             -bad recombine_results/${1}.1.bad.rpt \
                                             -out recombine_results/${1}.good.rpt)
        echo ${1}"	""${a}""	0"    >> final_summary.rpt
    fi
}
export -f summarize
