...
def find_overlap(fn_bam, list_gene_name, list_gene_position, ref_name, fn_out, flag_read_name, debug=False):
    ...
    for segment in f_bam.fetch(ref_name, J_region[0], J_region[1]):
        ...
        try:
            ...
            VD_gene = []
            C_gene = []
            for SA_tag in sorted(list_alignment):
            ...
            if len(VD_gene) > 0:
                ...
                if dict_bad_reads.get(seq_name): # first check if the alignment is good
                    dict_bad_reads[seq_name] = tuple(C_gene + [J_gene] + VD_gene)
                elif dict_VDJ_pairs.get(tuple(C_gene + [J_gene] + VD_gene)): # if the combination is already there
    ...
    # output stage
    if fn_out:
        f = open(fn_out + '.good.rpt', 'w')
        for combination, list_seq_name in sorted(dict_VDJ_pairs.items()):
            if combination == tuple('Unrecombined'):
                f.write('Unrecombined\t' + str(len(list_seq_name)) + '\n')
            else:
                f.write('---'.join(combination) + '\t' + str(len(list_seq_name)) + '\n')
        f.close()
        f = open(fn_out + '.bad.rpt', 'w')
        for name, combination in sorted(dict_bad_reads.items()):
            f.write(name + '\t' + '---'.join(combination) + '\n')
        f.close()
    else:
        for combination, list_seq_name in sorted(dict_VDJ_pairs.items()):
            if combination == tuple('Unrecombined'):
