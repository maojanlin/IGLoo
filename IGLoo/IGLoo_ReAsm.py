...
def main():
    ...
    if parent_1 == None:
        ...
        if parent_1[-4:] == ".bam":
            command = ' '.join(['samtools', 'fasta', parent_1, '>', parent_1[:-4]+".fa"])
            subprocess.call(command, shell=True)
            parent_1 = parent_1[:-4] + ".fa"
        if parent_2[-4:] == ".bam":
            command = ' '.join(['samtools', 'fasta', parent_2, '>', parent_2[:-4]+".fa"])
            subprocess.call(command, shell=True)
            parent_2 = parent_2[:-4] + ".fa"

        # Trio-binning mode of hifiasm
        command = ' '.join(['yak count -k31 -b37 -t16 -o', out_dir+'/parent/pat.yak', parent_1])
        subprocess.call(command, shell=True)
        command = ' '.join(['yak count -k31 -b37 -t16 -o', out_dir+'/parent/mat.yak', parent_2])
        subprocess.call(command, shell=True)
